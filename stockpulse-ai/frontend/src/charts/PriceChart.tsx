...
const PriceChart: React.FC<PriceChartProps> = ({ ticker, currentPrice }) => {
  ...
  const [predictionColor, setPredictionColor] = useState<string>('#A855F7');
      ...
      <div style={{ width: '100%', height: '300px', minHeight: '300px', paddingTop: '8px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            ...
            <XAxis dataKey="time" stroke="#8B95A5" fontSize={10} tickLine={false} axisLine={{ stroke: '#202630' }} />
            <YAxis domain={[minPrice, maxPrice]} stroke="#8B95A5" fontSize={10} tickLine={false} axisLine={{ stroke: '#202630' }} tickFormatter={(val) => `$${val}`} />
            <Tooltip
              contentStyle={{
                backgroundColor: '#0D1117',
                borderColor: '#202630',
            ...
            />
            <Line type="monotone" dataKey="price" stroke="#38BDF8" strokeWidth={2} dot={false} connectNulls={false} activeDot={{ r: 4, stroke: '#38BDF8', strokeWidth: 2, fill: '#0D1117' }} name="Price" />
            {showPrediction && (
              <Line
                type="monotone"
                dataKey="modelPred"
            ...
            {sma20 && <Line type="monotone" dataKey="sma20" stroke="#3B82F6" strokeDasharray="3 3" strokeWidth={1} dot={false} name="SMA20" />}
            {sma50 && <Line type="monotone" dataKey="sma50" stroke="#F59E0B" strokeDasharray="3 3" strokeWidth={1} dot={false} name="SMA50" />}
            {bollinger && (
              <>
                <Line type="monotone" dataKey="bbUpper" stroke="#4E5766" strokeDasharray="2 2" strokeWidth={1} dot={false} name="Upper Band" />
                <Line type="monotone" dataKey="bbLower" stroke="#4E5766" strokeDasharray="2 2" strokeWidth={1} dot={false} name="Lower Band" />
              </>
            )}
          </ComposedChart>
        </ResponsiveContainer>
